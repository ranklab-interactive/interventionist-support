...
jQuery("body").ready(function($) {
    ...
    $('a[href^="mailto"]').on('click', function(e) {
        _gaq.push(['_trackEvent', 'Email Address Clicks', this.href.replace(/^mailto:/i, '')]);
    });
});
...
jQuery("body").ready(function($) {
    ...
    //Old case sensitive version: $('a[href^="http"]:not([href*="//' + location.host + '"])').on('click', function(e) {
		...
		if(regex_pttn.test(this.href)) {
			_gaq.push(['_trackEvent', 'Downloads', regex_pttn.exec(this.href)[1].toUpperCase(), "Off-Site Download: "+this.href.replace(/^.*\/\//, '')]);
			//alert("Outbound Download Tracked: (" + regex_pttn.exec(this.href)[1].toUpperCase() + ") " + this.href.replace(/^.*\/\//, ''));
		};
		//alert($(this).attr("target"));
		//Tracks outbound links whether download or not. Creates duplicate events, but tagged differently.
		_gaq.push(['_trackEvent', 'Outbound Links', "Outbound: "+this.href.match(/\/\/([^\/]+)/)[1]]);
    });
...
jQuery("body").ready(function($) {
    ...
    $('a:regex(href,\\.(zip|mp\\d+|mpe*g|pdf|docx*|pptx*|xlsx*|jpe*g|png|gif|tiff*))').on('click', function(e) {
		...
		if (this.href.indexOf(baseDomain)>0) {
			regex_pttn = /\.(zip|mp\\d+|mpe*g|pdf|docx*|pptx*|xlsx*|jpe*g|png|gif|tiff*)/i;
			_gaq.push(['_trackEvent', 'Downloads', regex_pttn.exec(this.href)[1].toUpperCase(), "On-Site Download: "+this.href.replace(/^.*\/\//, '')]);
...
if (document.title.search(error_404_title) >= 0) {
	...
	_gaq.push(['_trackEvent', '404 Errors', error_type + "; Broken_URL=" + location.pathname + $site_referrer, error_label]);  //removed: + '?from=' + document.referrer
