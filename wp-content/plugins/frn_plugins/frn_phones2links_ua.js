...
jQuery(document).ready(function() {
	if(isTierIphone || isTierTablet) {
		...
		if((typeof frn_phone_autoscan != 'undefined' && typeof phone_spans == 'undefined')  ) {  //  && (isTierIphone || isTierTablet)
			//if a span using our frn_phone ID is found on the page, this is disabled to avoid duplicate links
			//In the future, we can add this to find the parent DOM and see if it's a SPAN with that id. If not, then continue or run the normal process.
			...
			for (var n = 0, len = matches.length; n < len; n++) {
				...
				//var parent_dom = matches[n].parentNode;
				//alert(parent_dom.getAttribute("id"));
				ga_category="Phone Numbers";
				ga_action="Phone Clicks (General)";
				ga_label=phone_actual;
				ga_trigger="ga('send', 'event', '" +ga_category+ "', '"+ga_action+"', '" +ga_label+ "');";
			   ...
			   if(all_spans[i].getAttribute("id")!=null) {
				   if(all_spans[i].getAttribute("id")=="frn_phones") {
					...
					if(frn_number==null && inner_code!=null) {
						//If number is not defined by a custom attribute, then search between the SPAN tags for a phone number pattern
					...
					//if((isTierIphone || isTierTablet || inner_code.trim()=="") && frn_number!="") { //	
						...
						if(frn_number!="")  {
							...
							if(frn_number.length==11) {frn_phoneFormat = /^1?(\d)([0-9]..)([0-9]..)([0-9]...)$/; //starts with 1
							  ...
							  frn_number_mobile = frn_number.replace(frn_phoneFormat,'$1-$2-$3-$4');}
							else if(frn_number.length>=14 && frn_number.slice(0,1)!="("){frn_phoneFormat = /([0-9]...)([0-9]..)([0-9]..)([0-9]...)$/; //international
							  frn_number_mobile = frn_number.replace(frn_phoneFormat,'$1-$2-$3-$4');}
						}
